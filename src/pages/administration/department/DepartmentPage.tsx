...
const DepartmentPage = () => {
  ...
  const getData = async (payload: { filtering?: any, pagnation: { page: number, limit: number }, sort?: any, }) => {
    ...
    let url = `/department/fetch?search=${payload.filtering ? payload.filtering : ''}&page=${page}&limit=${limit}`;
    if (payload.sort) {
  ...
  const table: any = useReactTable({
    data: departments.data ? departments.data : [],
    columns,
    state: {
      sorting,
    ...
    pageCount: Math.ceil(departments.totalCount / departments.limit),
    ...
    <>
      <div className="flex mb-2 justify-between">
        ...
        <div className="relative">
          ...
          <input type='text' value={filtering} onChange={e => setFiltering(e.target.value)} className="focus:outline-teal-500 block w-72 p-2 pl-10 text-sm text-gray-600 border border-gray-300 rounded-lg bg-white   " placeholder="Search Department Name ..." required />
        </div>
      ...
      <div>
        <ul>
          {table.getSelectedRowModel().flatRows.map((el: any) => {
