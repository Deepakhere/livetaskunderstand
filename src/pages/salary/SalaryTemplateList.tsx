...
const SalaryTemplateList = () => {
  ...
  const [salaryData, setSalaryData]: any = useState([]);
  ...
  const table: any = useReactTable({
    data: salaryData.data ? salaryData.data : [],
    columns,
    state: {
      sorting,
    ...
    pageCount: Math.ceil(salaryData.totalCount / salaryData.limit),
  ...
  const getData = async (payload: { filtering?: any, pagnation: { page: number, limit: number }, sort?: any, }) => {
    ...
    let url = `salary-template/fetch?search=${payload.filtering ? payload.filtering : ''}&page=${page}&limit=${limit}`;
    if (payload.sort) {
    ...
    <>
      <div className="flex mb-2 justify-between">
        ...
        <div className="relative">
          ...
          <input type='text' value={filtering} onChange={e => setFiltering(e.target.value)} className="focus:outline-teal-500 block w-72 p-2 pl-10 text-sm text-gray-600 border border-gray-300 rounded-lg bg-white   " placeholder="Enter Pay Grade Name..." required />
        </div>
